(function() {
	var h = {},
		mt = {},
		c = {
			id: "ffe6968809d7872701e07ea21cce5427",
			dm: ["xiumi.us"],
	...
	(mt.p.X = function() {
		...
		(function() {
			if(!e)
				if(e = r, "complete" === document.readyState) a.z = r;
		...
		return function(f) {
			switch(typeof f) {
				...
				default:
					...
					p = mt.j.stringify;
					for(m in f)
						if(Object.prototype.hasOwnProperty.call(f, m)) switch(g =
							f[m], typeof g) {
							case "undefined":
							case "unknown":
							case "function":
	...
	(function() {
		...
		function d() {
			...
			if((!n || !g) && p && b) v = r, l = +new Date;
			else if(n && g && (!p || !b)) v = u, t += +new Date - l;
			n = p;
			g = b;
			z = setTimeout(d, 100)
		}
	...
	(function() {
		...
		var f =
			...
			g = {
				...
				ia: function() {
					if(0 < g.r.length)
						for(var b = 0, a = g.r.length; b < a; b++) g.M(g.r[b]);
					g.r = s
				},
				M: function(b) {
					var a = b[0];
				...
				_trackRTEvent: function(b) {
					...
					if(f.d(b, "Object")) {
						...
						if(900 >= l) a.call(this, b);
							...
							for(var l = Math.ceil(l / 900), e = "block|" + Math.round(Math.random() * m.L).toString(16) + "|" + l + "|", v = [], x = 0; x < l; x++) v.push(x), v.push(b.substring(900 * x, 900 * x + 900)), a.call(this, e + v.join("|")), v = []
					}
				},
		...
		a.prototype = {
			...
			H: function() {
				for(var b = document.location.hostname, a = 0, d = c.dm.length; a < d; a++)
					if(this.J(b, c.dm[a])) return c.dm[a].replace(/(:\d+)?[\/\?#].*/, "");
				return b
			},
			R: function() {
				...
				var v = function(a, b) {
					...
					if(d.d(b, "Array") || d.d(b, "NodeList"))
						for(var e = 0, f = b.length; e < f; e++)
							if(t === b[e]) return w(m + a + "/" + (e + 1), b[e]), u
				};
