 ...
 * @Last Modified time: 2017-05-22 21:01:44
 */
var product_name;
var company_name;
var product_img;
...
function computeMinimunCoverage(_r, _age, _E, _curC){
    ...
    return parseInt(s2*C/(t1*s1+t2*s2)/10000);
}
...
function calcUniveralAccount(){
    ...
    if(v_age<age){
        ...
        result = A*Math.pow(1+r, n+1)+t2*(Math.pow(1+r, n+1)-1)/r;
    }
    else{
        var A = t1*coverage*(Math.pow(1+r, 59-age)-1)/r;
        var B = A*Math.pow(1+r, 80-60+1)+t2*(Math.pow(1+r, 80-60+1)-1)/r;

        var n = v_age-80;

        result = B*Math.pow(1+r, n);
    }
...
function calcAnnuities(){
    ...
    var _r  =0.045;
    var C = coverage;
    var A = t1*coverage*(Math.pow(1+_r, m-1-age)-1)/_r;

    var annuity;
    if(i>n){
    var h = Math.pow(1+_r, n-m);
    annuity = A*_r*h/(h-1)+t2*coverage;
    }
    else{
        var K = i-m;
        var L = n-i;

        var _left = A*10000*Math.pow(1+_r, n-m)*r+period*premium*Math.pow(1+_r, L)*r-t2*coverage*10000*(Math.pow(1+_r, L)-1);
        annuity = _left/(Math.pow(1+_r, L)-1)+t2*coverage;
    }
