...
if($row)
 {
     ...
     echo "<company_name>" . $row['company'] . "</company_name>";
 }
 else
 {
...
//   附加险
$sql = sprintf("SELECT * FROM tbl_product_basic_info WHERE main_risk='%s'", $product_name);

$retval = mysql_query($sql, $conn) or die(mysql_error());
$row = mysql_fetch_array($retval);
$accessory="";
if($row)
     $accessory=$row['product_name'];

while($row = mysql_fetch_array($retval))
     $accessory = $accessory."-".$row['product_name'];

echo "<accessory_risk>".$accessory."</accessory_risk>";
